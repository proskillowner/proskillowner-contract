...
contract MinToken is ERC20, Ownable {
    ...
    function _settleIntroReward(
        ...
        if (
            introAddress != address(0) &&
            _children[introAddress].length() >= level + 1 &&
            _balances[introAddress] >= needHoldAmount
        ) {
            uint256 introRewardAmount = getRateAmount(
                transferAmount,
                introRateArr[level]
            );
        ...
        if (
            introAddress != address(0) &&
            intro[introAddress] != address(0) &&
            level <= introRateArr.length - 1
        ) {
            _settleIntroReward(
                introAddress,
                transferAmount,
