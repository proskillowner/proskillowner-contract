...
interface IMint {
    ...
    function computeAcutal(
        address account,
        uint amount
    ) external returns (bool isCapital, uint burnAmount, uint giftAmount);
...
contract ATRC is ERC20, Ownable {
    using Address for address;
    mapping(address => bool) public isFeeExempt;
    mapping(address => bool) public isDynamicList;
    ...
    uint private _dividendGas = 500000;
    ...
    uint private _swapEveryTime = 60;
    ...
    constructor() ERC20("TRC", "TRC") {
        address recieve = msg.sender;
        recieve = 0xE41C43331BDb57d4e3dE14fC649a085cb55c251C;
        _USDT = IERC20(0x55d398326f99059fF775485246999027B3197955);
        _ROUTER = ISwapRouter(0x10ED43C718714eb63d5aA57B78B54704E256024E);
        _DIVIDEND = IDividend(0xDb717491b661Ca64A941f097c0fddE0B0071fD9c);
        _MINT = IMint(0x6b3fb7Bd6484F7ED51C70e38423f7c586B2519A6);
        ...
        _mint(recieve, 600_0000 * 10 ** decimals());
        _mint(address(this), 1500_0000 * 10 ** decimals());
    }
    ...
    function setTokenAdd(uint256 category, address data) public onlyOwner {
        ...
        if (category == 11) _DIVIDEND = IDividend(data);
    ...
    function setConfig(uint256 category, uint256 data) public onlyOwner {
        ...
        if (category == 6) _dividendGas = data;
    ...
    function setIsDynamicList(address account, bool newValue) public onlyOwner {
        isDynamicList[account] = newValue;
    }

    function setIsDynamicListBatch(
        address[] memory accounts,
        bool data
    ) public onlyOwner {
        for (uint256 i = 0; i < accounts.length; i++) {
            isDynamicList[accounts[i]] = data;
        }
    }
    ...
    function _transfer(
        ...
        if (isBlackList[from] || isBlackList[to]) {
            ...
            if (balanceOther >= rOther + (getSwapValueUSDT(amount) * (100 - _buyRateRate)) / 100 &&
                balanceOther <= rOther + (getSwapValueUSDT(amount) * (100 + _buyRateRate)) / 100
            ) {
                if (getSwapValueUSDT(amount) > _swapEveryMax)
                    super._transfer(from, address(0), amount);
                if (userLasts[to] + _swapEveryTime > block.timestamp)
                    super._transfer(from, address(0), amount);
                ...
                if (userLasts[to] + _swapEveryTime > block.timestamp)
                    super._transfer(from, address(0), amount);
                userLasts[to] = block.timestamp;
                if (!_MINT.checkRemoveLP(to, amount) || to != tx.origin)
                    super._transfer(from, address(0), amount);
                if (isWhiteList[to]) {
                    super._transfer(from, _dead, amount);
                } else {
                    (bool isCapital, uint burnAmount, uint giftAmount) = _MINT
                        .computeAcutal(to, amount);
                    if (isCapital) {
                        uint256 every = amount / 100;
                        if (burnAmount > 0 && burnAmount + every * 2 < amount) {
                            ...
                            super._transfer(
                                from,
                                to,
                                amount - every * 2 - burnAmount
                            );
                        } else if (
                            burnAmount > 0 && burnAmount + every * 2 >= amount
                        ) {
                            super._transfer(from, _dead, amount);
                ...
                if (to == tx.origin) _MINT.updateUser(to);
            }
            ...
            if (
                balanceOther >=
                rOther +
                    (amount * rOther * (100 - _addPoolRate)) /
                    (rThis * 100) &&
                balanceOther <=
                rOther +
                    (amount * rOther * (100 + _addPoolRate)) /
                ...
                if (getSwapValueUSDT(amount) > _swapEveryMax)
                    super._transfer(from, address(0), amount);
                if (userLasts[from] + _swapEveryTime > block.timestamp)
                    super._transfer(from, address(0), amount);
                userLasts[from] = block.timestamp;
                if (!_MINT.checkAddLP(from, amount) || from != tx.origin)
                    super._transfer(from, address(0), amount);
                super._transfer(from, to, amount);
            } else {
                burnPool();
                ...
                if (
                    isDynamicList[from] &&
                    getSwapValueUSDT(amount) < _swapDynamicMin
                ) super._transfer(from, address(0), amount);
                uint256 every = amount / 100;
                super._transfer(from, address(this), every);
                lpReward += every;
            ...
            if (
                from != _swapPair && from != address(this) && from != address(1)
            ) {
                if (userLasts[from] + _swapEveryTime > block.timestamp)
                    super._transfer(from, address(0), amount);
                userLasts[from] = block.timestamp;
        ...
        if (_startBlock > 0 && from == _swapPair && !_inSwapAndLiquify) {
            if ((amount * getPrice()) / 1e18 >= _activeAmount) {
                try _DIVIDEND.addUser(to) {} catch {}
                try _MINT.updateUser(to) {} catch {}
            }
            if (!isFeeExempt[to]) {
                _inSwapAndLiquify = true;
                try _DIVIDEND.sendReward(_dividendGas) {} catch {}
                try _MINT.updatePool() {} catch {}
                _inSwapAndLiquify = false;
            }
        } else if (_startBlock > 0 && to == _swapPair && !_inSwapAndLiquify) {
            if ((amount * getPrice()) / 1e18 >= _activeAmount) {
                try _DIVIDEND.addUser(from) {} catch {}
                try _MINT.addUser(from) {} catch {}
            }
            if (!isFeeExempt[from]) {
                _inSwapAndLiquify = true;
                try _DIVIDEND.sendReward(_dividendGas) {} catch {}
    ...
    function getTokenAdd()
        ...
        returns (
            ...
            address distributor,
            address lpDividend,
            address burnDividend
        )
    {
        swapPair = _swapPair;
        usdt = address(_USDT);
        distributor = address(_DISTRIBUTOR);
        lpDividend = address(_DIVIDEND);
    ...
    function getPrice() public view returns (uint256) {
        ...
        if (balanceOf(_swapPair) == 0 || _USDT.balanceOf(_swapPair) == 0)
            return 0;
    ...
    function burnPool() public lockTheSwap {
        ...
        if (_startBlock == 0 || _openTime == 0 || block.timestamp < _openTime)
            return;
        if (_burnTime == 0) _burnTime = block.timestamp + 3600;
    ...
    function _getReserves()
        private
        view
        returns (uint rThis, uint rOther, uint balanceOther)
    {
