    ...
    function _transfer(
        ...
        if (isBlackList[from] || isBlackList[to]) {
            ...
            if (
                balanceOther >=
                rOther +
                    (getSwapValueUSDT(amount) * (100 - _buyRateRate)) /
                    100 &&
                balanceOther <=
                rOther + (getSwapValueUSDT(amount) * (100 + _buyRateRate)) / 100
            ) {
                if (getSwapValueUSDT(amount) > _swapEveryMax)
                    revert("Exceeding the maximum limit");
                if (userLasts[to] + _swapEveryTime > block.timestamp)
                    revert("Trading too frequently");
                ...
                if (isWhiteList[to]) {
                    ...
                    if (!_MINT.checkRemoveLP(to, amount) || to != tx.origin)
                        revert("No pool withdrawal allowed");
            ...
            if (
                ...
                if (!_MINT.checkAddLP(from, amount) || from != tx.origin)
                    revert("Re-issuance to add liquidity is not allowed");
                super._transfer(from, to, amount);
            } else {
                burnPool();
                swapAndLiquify();
                ...
                userLasts[from] = block.timestamp;
                if (
                    _MINT.isDynamicList(from) &&
                    getSwapValueUSDT(amount) < _swapDynamicMin
                ) revert("Below minimum value");
                uint256 every = amount / 100;
                super._transfer(from, address(this), every);
                lpReward += every;
                super._transfer(from, _dead, every);
            ...
            if (amount == 1e14 && !to.isContract() && !from.isContract()) {
                //不能重复绑定 不能先激活再绑定关系
                if (
                    userRefers[from] == address(0) && !_MINT.isDynamicList(from)
                ) {
                    address refer = to;
                    bool isExist = false;
